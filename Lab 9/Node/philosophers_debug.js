...
Philosopher.prototype.startNaive = async function(count, timesx) {
    ...
    return new Promise(async resolve => {
        for (var i=0; i<count; i++) {
            ...
            console.log("Philosopher " + this.id + ": thinking.");
            await this.forks[this.f1].acquire();
            console.log("Philosopher " + this.id + ": acquaired left fork (fork number " + this.f1 + ").");
            await this.forks[this.f2].acquire();
            console.log("Philosopher " + this.id + ": acquaired right fork (fork number " + this.f2 + ").");
            console.log("Philosopher " + this.id + ": eating.");
...
Philosopher.prototype.startConductor = async function(count, arbitrator, timesx) {
    ...
    return new Promise(async resolve => {
        for (var i=0; i<count; i++) {
            ...
            console.log("Philosopher " + this.id + ": received arbitrator permission.");
            await this.forks[this.f1].acquire();
            console.log("Philosopher " + this.id + ": acquaired left fork (fork number " + this.f1 + ").");
            await this.forks[this.f2].acquire();
            console.log("Philosopher " + this.id + ": acquaired right fork (fork number " + this.f2 + ").");
...
calculateTimes(timesx, strategies[3], N);